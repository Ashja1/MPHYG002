...
struct Graph
{
	// V-> Number if vertices, E-> Number of edges
	int V,E;
	// graph is represented as an array of edges.Since the graph is undirected
	// the edge from src to dest is also edge from dest
...
void KrushkalMST(struct Graph* graph)
{
	...
	int e =0;// An index variable, used for resut[]
	int i=0; // An index varible, used for sorted edges
	...
	// print the contents of result[] to display the bult MST
...
void process_krushkal()
{
	...
	*/
	...
	//testing filehandling.cpp
	//string s  = "example.txt";
	//vector<int> res = readFile();
	

	int V = 4; // Number if vertices in graph
	int E = 5; // Number of edges in graph
	struct Graph* graph = createGraph(V,E);
	
	//for(std::vector<int>::size_type i=0; i!= res.size();i++){
	//	cout<<"\n res["<<i<<"]="<<res[i];
		/*graph->edge[i].src = res[i];
		graph->edge[i].dest =res[i+1];
		graph->edge[i].dest = res[i+1];
		*/
